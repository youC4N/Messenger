...
extension API {
    func fetchAvatar(ofUser userID: UserID, sessionToken: SessionToken) async throws
        -> FetchAvatarResponse
    {
        var request = URLRequest(
            url: endpoint.appending(components: "user", userID.description, "avatar"))
        ...
        guard let contentType = httpResponse.value(forHTTPHeaderField: "Content-Type") else {
            throw ServerRequestError.unexpectedResponse(
                message: "Expected content-type header to be set on an avatar response")
        }
