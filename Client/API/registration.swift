...
extension API {
    func registerUser(
        registrationToken token: RegistrationToken, username: String, avatar: FileForUpload<Data>?
        ...
        if let avatar = avatar {
            parts.append(
                .file(name: "avatar", bytes: avatar.bytes, contentType: avatar.contentType))
        }
        ...
        guard httpResponse.statusCode != 400 else {
            let errorResponse = try JSONDecoder().decode(
                ErrorResponse<RegistrationResponse.ErrorKind>.self, from: body)
...
private func multipartEncode(_ values: [MultipartPart]) -> (body: Data, contentTypeHeader: String) {
    ...
    for part in values {
        ...
        case .field(let name, let value):
            ...
            data.append(
                contentsOf:
                    "Content-Disposition: form-data; name=\(String(reflecting: name))\r\n\r\n".utf8)
            data.append(contentsOf: value.utf8)
        case .file(let name, let bytes, let contentType, let filename):
            data.append(contentsOf: "\r\n--\(boundary)\r\n".utf8)
            data.append(
                contentsOf: "Content-Disposition: form-data; name=\(String(reflecting: name))".utf8)
