...
func routes(_ app: Application, db: Connection) throws {
    ...
    app.get("bazinga", ":id") { req async throws in
        ...
        guard
            let row = try db.prepare(
                "select id, name from bazinga where id = ? limit 1", id
            )
            .firstRow()
        else {
    ...
    app.post("bazinga") { req async throws in
        ...
        guard
            let res = try db.prepare(
                "insert into bazinga (name) values (?) returning id, name",
                createRequest.name
            )
            .firstRow()
        else { throw MessangerError.serverError }
